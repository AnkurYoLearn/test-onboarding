...
const ChatBot: React.FC = () => {
  const {
    ...
    nextStep,
    setLoading,
    setUserType // <-- add this from context
  } = useOnboarding();
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [currentInput, setCurrentInput] = useState('');
  const [waitingForResponse, setWaitingForResponse] = useState(false);
  const [selectedOptions, setSelectedOptions] = useState<string[]>([]);
  const [inputType, setInputType] = useState<'text' | 'select' | 'multiselect'>('text');
  ...
  const initializeOnboarding = async () => {
    ...
    if (userId && userTypeParam && (userTypeParam === 'student' || userTypeParam === 'teacher')) {
      ...
      addMessage('Welcome to YoLearn! Please provide your user ID and select your type to continue with onboarding.', 'question');
      setInputType('text');
    }
  };

  const startOnboardingFromParams = async (userId: string, userTypeParam: 'student' | 'teacher') => {
    try {
      setLoading(true);
      ...
      addMessage('Sorry, there was an error starting your onboarding. Please try again.', 'question');
    } finally {
      setLoading(false);
    }
  };

  const checkOnboardingStatusAndInitialize = async (userId: string, userType: string) => {
    try {
      ...
      if (statusResult.success && statusResult.data?.data) {
        ...
        if (completed && profile) {
          ...
          setShowProfile(true);
          addMessage(`Welcome back! Your ${userType} profile is complete.`, 'question');
        } else if (has_data) {
          // User has partial data
          addMessage(`Welcome back! Let's continue your ${userType} onboarding.`, 'question');
      ...
      addMessage(`Hello! Welcome to YoLearn ${userType} onboarding. What should I call you?`, 'question');
      setInputType('text');
    }
  };
  ...
  const addMessage = (content: string, type: ChatMessage['type'] = 'question', options?: string[]) => {
    ...
    if (!hasCompletedOnboarding) {
      ...
      addMessage('Hello! Welcome to YoLearn. What should I call you?', 'question');
      setInputType('text');
    }
  };
  ...
  const handleNext = async () => {
    ...
    } else if ((inputType === 'select' || inputType === 'multiselect') && selectedOptions.length > 0) {
      let finalOptions = [...selectedOptions];
      if (selectedOptions.includes('Others (please specify)') && othersText.trim()) {
        finalOptions = finalOptions.filter(o => o !== 'Others (please specify)');
    ...
    try {
      if (!currentUserType) {
        addMessage('Please start onboarding with valid user credentials first.', 'question');
      ...
      if (!userName) {
        ...
        addMessage(`Nice to meet you, ${response}! Which country are you ${currentUserType === 'student' ? 'studying' : 'teaching'} in?`, 'question');
        setInputType('text');
        setCurrentInput('');
        return;
      }
  ...
  const handleStudentStep = async (response: string) => {
    ...
    switch (currentStep) {
      case 1: // Country
        ...
        try {
          const curricula = await studentAPI.getCurricula({ country: response });
          toast.success("Saved.", { id: "status" })
          addMessage('Which curriculum are you following?', 'question', curricula.options);
          setInputType('select');
        } catch (error) {
          addMessage("I couldn't fetch curriculum options. Please try again.", 'question');
        }
        nextStep();
        break;
      case 2: // Curriculum
        ...
        try {
          const grades = await studentAPI.getGradesByCurriculum({ curriculum: response });
          toast.success("Saved.", { id: "status" })
          addMessage('What grade are you in?', 'question', grades.options);
          setInputType('select');
        } catch (error) {
          addMessage("I couldn't fetch grade options. Please try again.", 'question');
        }
        nextStep();
        break;
      case 3: // Grade
        ...
        try {
          ...
          toast.success("Saved.", { id: "status" })
          addMessage('Which subjects are you studying? (You can select multiple)', 'question', subjects.options);
          setInputType('multiselect');
        } catch (error) {
          addMessage("I couldn't fetch subjects. Please try again.", 'question');
        }
        nextStep();
        break;
      case 4: // Subjects
        const subjects = response.includes(',') ? response.split(', ') : [response];
        ...
        try {
          ...
          toast.success("Saved.", { id: "status" })
          addMessage('What are your learning interests? (Select multiple if you like)', 'question', interests.options);
          setInputType('multiselect');
        } catch (error) {
          addMessage("I couldn't fetch learning interests. Please try again.", 'question');
        }
        nextStep();
        break;
      case 5: // Learning Interests
        const interests = response.includes(',') ? response.split(', ') : [response];
        ...
        try {
          ...
          toast.success("Saved.", { id: "status" })
          addMessage("What's your preferred learning style?", 'question', styles.options);
          setInputType('select');
        } catch (error) {
          addMessage("I couldn't fetch learning styles. Please try again.", 'question');
        }
        nextStep();
        break;
      case 6: // Learning Styles
        ...
        try {
          ...
          toast.success("Saved.", { id: "status" })
          addMessage("How do you prefer to get help when you're stuck?", 'question', helpPrefs.options);
          setInputType('select');
        } catch (error) {
          addMessage("I couldn't fetch help preferences. Please try again.", 'question');
        }
        nextStep();
        break;
      case 7: // Help Preferences
        ...
        try {
          ...
          toast.success("Saved.", { id: "status" })
          addMessage('What are your main learning goals? (Select multiple)', 'question', goals.options);
          setInputType('multiselect');
        } catch (error) {
          addMessage("I couldn't fetch learning goals. Please try again.", 'question');
        }
        nextStep();
        break;
      case 8: // Learning Goals
        const goals = response.includes(',') ? response.split(', ') : [response];
        ...
        try {
          ...
          if (saveResult.success) {
            toast.success("Saved.", { id: "status" })
            addMessage("🎉 Fantastic! Your student profile is complete and saved. Here's your personalized learning profile:", 'question');
          } else {
            toast.error("Cannot save.", { id: "status" })
            addMessage("🎉 Fantastic! Your student profile is complete. Here's your personalized learning profile:", 'question');
            console.error('Failed to save complete onboarding:', saveResult.error);
          }
        } catch (error) {
  ...
  const handleTeacherStep = async (response: string) => {
    ...
    switch (currentStep) {
      case 1: // Country (start with country, not name)
        ...
        addMessage("Loading curriculum options...", 'loading');
        try {
          console.log('🔄 Calling teacherAPI.getCurricula with:', { country: response, grade_levels: '' });
          ...
          addMessage("Which curriculum do you follow?", 'question', curricula.options);
          setInputType('select');
        } catch (error) {
          toast.error("Cannot save.", { id: "status" })
          console.error('❌ Error fetching teacher curricula:', error);
          addMessage("I couldn't fetch curricula. Please try again.", 'question');
        }
        nextStep();
        break;      case 2: // Curriculum
        updateTeacherData({ selected_curriculum: response });
        addMessage("Loading grade level options...", 'loading');
        try {
          const gradeLevels = await teacherAPI.getGradeLevels({ country: teacherData.country || '' });
          toast.success("Saved.", { id: "status" })
          addMessage("Which grade levels do you teach?", 'question', gradeLevels.options);
          setInputType('select');
        } catch (error) {
          addMessage("I couldn't fetch grade levels. Please try again.", 'question');
        }
        nextStep();
        break;case 3: // Grade Levels
        ...
        try {
          ...
          toast.success("Saved.", { id: "status" })
          addMessage("Which subjects do you teach? (Select multiple)", 'question', subjects.options);
          setInputType('multiselect');
        } catch (error) {
          addMessage("I couldn't fetch subjects. Please try again.", 'question');
        }
        nextStep();
        break;      case 4: // Subjects
        const teacherSubjects = response.includes(',') ? response.split(', ') : [response];
        ...
        try {
          ...
          toast.success("Saved.", { id: "status" })
          addMessage("What are your main teaching goals? (Select multiple)", 'question', goals.options);
          setInputType('multiselect');
        } catch (error) {
          addMessage("I couldn't fetch teaching goals. Please try again.", 'question');
        }        nextStep();
        break;      case 5: // Teaching Goals
        const teachingGoals = response.includes(',') ? response.split(', ') : [response];
        ...
        try {
          ...
          toast.success("Saved.", { id: "status" })
          addMessage("How comfortable are you with technology?", 'question', techComfort.options);
          setInputType('select');
        } catch (error) {
          addMessage("I couldn't fetch tech comfort options. Please try again.", 'question');
        }
        nextStep();
        break;      case 6: // Tech Comfort
        ...
        try {
          ...
          toast.success("Saved.", { id: "status" })
          addMessage("What are your lesson plan preferences?", 'question', lessonPlans.options);
          setInputType('select');
        } catch (error) {
          addMessage("I couldn't fetch lesson plan preferences. Please try again.", 'question');
        }
        nextStep();
        break;      case 7: // Lesson Plan Preferences
        ...
        try {
          ...
          toast.success("Saved.", { id: "status" })
          addMessage("What kind of device access do your students have?", 'question', deviceAccess.options);
          setInputType('select');
        } catch (error) {
          addMessage("I couldn't fetch device access options. Please try again.", 'question');
        }
        nextStep();
        break;      case 8: // Device Access
        ...
        try {
          ...
          const selected_subjects = Array.isArray(teacherData.selected_subjects)
            ...
            : typeof teacherData.selected_subjects === 'string' && teacherData.selected_subjects
              ? (teacherData.selected_subjects as string).split(',').map((s: string) => s.trim())
              : [];
          const selected_teaching_goals = Array.isArray(teacherData.selected_teaching_goals)
            ? teacherData.selected_teaching_goals
            : typeof teacherData.selected_teaching_goals === 'string' && teacherData.selected_teaching_goals
              ? (teacherData.selected_teaching_goals as string).split(',').map((s: string) => s.trim())
              : [];
          // Use comma (no space) for backend compatibility
          const selected_grade_levels = Array.isArray(teacherData.selected_grade_levels)
            ? teacherData.selected_grade_levels.join(',')
            : (typeof teacherData.selected_grade_levels === 'string' ? teacherData.selected_grade_levels : '');
          const selected_device_access = Array.isArray(teacherData.selected_device_access)
          ...
          const saveResult = await saveCompleteOnboarding(completeData);
          if (saveResult.success) {
            addMessage("🎉 Perfect! Your teacher profile is complete and saved. Here's your personalized teaching profile:", 'question');
          } else {
            addMessage("🎉 Perfect! Your teacher profile is complete. Here's your personalized teaching profile:", 'question');
            console.error('Failed to save complete onboarding:', saveResult.error);
          }
        } catch (error) {
          console.error('Error saving complete onboarding:', error);
  ...
  const canProceed = () => {
    if (inputType === 'text') {
      ...
      const needsOthersText = selectedOptions.includes('Others (please specify)') && !othersText.trim();
  ...
  const currentQuestionMessage = messages.filter(m => m.type === 'question').slice(-1)[0];
          ...
          {inputType === 'text' ? (
            <div style={{ width: '100%', maxWidth: '420px', position: 'relative' }}>
              <Input
                ...
                placeholder={currentStep === 1 && userName ? 'Type or select your country...' : 'Enter answer'}
                onKeyPress={(e) => {
              ...
              />
              {currentStep === 1 && userName && showCountryDropdown && filteredCountries.length > 0 && (
                <div style={{
                  position: 'absolute',
                  top: '100%',
                  left: 0,
        ...
        <FooterBar>
          <div style={{
            display: "flex"
              ...
              <button
                onClick={handleNext} disabled={!canProceed() || waitingForResponse} 
                style={{
                  backgroundColor: "#fff",
                  border: "1px solid #00000033",
...
const countries: string[] = [
  "India", "United States", "United Kingdom", "Canada", "Australia", "Kenya", "Nigeria", "South Africa", "Singapore", "Germany", "France", "Brazil", "China", "Japan", "Russia", "Mexico", "Italy", "Spain", "Turkey", "Indonesia", "Pakistan", "Bangladesh", "Philippines", "Vietnam", "Egypt", "Argentina", "Poland", "Netherlands", "Saudi Arabia", "Malaysia", "Thailand", "Sweden", "Switzerland", "Belgium", "Greece", "Portugal", "Czech Republic", "Hungary", "Romania", "Ukraine", "Chile", "Colombia", "Peru", "Venezuela", "Morocco", "Algeria", "Ethiopia", "Ghana", "Tanzania", "Uganda", "Sri Lanka", "Nepal", "New Zealand", "Ireland", "Denmark", "Finland", "Norway", "Austria", "Israel", "South Korea", "UAE", "Qatar", "Kuwait", "Oman", "Jordan", "Lebanon", "Iraq", "Iran", "Afghanistan", "Myanmar", "Cambodia", "Laos", "Mongolia", "Kazakhstan", "Uzbekistan", "Turkmenistan", "Georgia", "Armenia", "Azerbaijan", "Slovakia", "Slovenia", "Croatia", "Serbia", "Bulgaria", "Estonia", "Latvia", "Lithuania", "Luxembourg", "Iceland", "Malta", "Cyprus", "Costa Rica", "Panama", "Uruguay", "Paraguay", "Bolivia", "Ecuador", "Guatemala", "Honduras", "El Salvador", "Nicaragua", "Jamaica", "Trinidad and Tobago", "Barbados", "Bahamas", "Fiji", "Papua New Guinea", "Zimbabwe", "Zambia", "Botswana", "Namibia", "Mozambique", "Angola", "Senegal", "Ivory Coast", "Cameroon", "Congo", "Sudan", "Libya", "Tunisia", "Syria", "Yemen", "Palestine", "North Korea"
];
